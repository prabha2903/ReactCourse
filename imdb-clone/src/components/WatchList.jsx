...
function WatchList({watchList,setWatchList,handleRemoveFromWatchList}) {
  ...
  return (
    ...
    <div className='flex justify-center flex-wrap m-4'>
      {genreList.map((genre)=>{
           return <div onClick={()=>handleFilter(genre)} className={currGenre==genre? 'flex justify-center items-center h-[3rem] w-[9rem] bg-blue-400 rounded-xl text-white font-bold mx-4': 
            'flex justify-center items-center h-[3rem] w-[9rem] bg-gray-400 rounded-xl text-white font-bold mx-4'}>{genre}</div>
      })}
    </div>
      ...
      <table className='w-full text-gray-500 text-center'>
        ...
        {watchList.filter((movieObj)=>{
          ...
          return movieObj.title.toLowerCase().includes(search.toLocaleLowerCase())
            ...
            return <tr key={movieObj.id} className='border-b-2 border-gray-300'>
